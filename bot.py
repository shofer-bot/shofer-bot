...
def init_storage():
    ...
    initial_students = [
        {"id": 1, "group": "Група 1", "name": "Грановський Даніїл Костянтинович", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 22, "paid_hours": 22, "school_payments": [{"hours": 22, "amount": 22 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 2, "group": "Група 1", "name": "Гаібова Вероніка Хуршедівна", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 26, "paid_hours": 26, "school_payments": [{"hours": 26, "amount": 26 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 3, "group": "Група 1", "name": "Криворот Єгор Олександрович", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 16, "paid_hours": 16, "school_payments": [{"hours": 16, "amount": 16 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 4, "group": "Група 1", "name": "Мосейчук Єлізавета Вадимівна", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 6, "paid_hours": 6, "school_payments": [{"hours": 6, "amount": 6 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 5, "group": "Група 1", "name": "Абрамова Анна Леонідівна", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 4, "paid_hours": 4, "school_payments": [{"hours": 4, "amount": 4 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 6, "group": "Група 1", "name": "Подплетньов Олексій Васильович", "phone": "Не вказано", "car_type": "citroen", "total_hours": 40, "spent_hours": 36, "paid_hours": 36, "school_payments": [{"hours": 36, "amount": 36 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 7, "group": "Група 1", "name": "Зіміна Аріна Володимирівна", "phone": "Не вказано", "car_type": "hyundai", "total_hours": 40, "spent_hours": 28, "paid_hours": 28, "school_payments": [{"hours": 28, "amount": 28 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 8, "group": "Група 1", "name": "Алексеєва Марія Владиславівна", "phone": "Не вказано", "car_type": "hyundai", "total_hours": 40, "spent_hours": 32, "paid_hours": 32, "school_payments": [{"hours": 32, "amount": 32 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 9, "group": "Група 1", "name": "Брьохова Софія Олександрівна", "phone": "Не вказано", "car_type": "hyundai", "total_hours": 40, "spent_hours": 26, "paid_hours": 26, "school_payments": [{"hours": 26, "amount": 26 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None},
        {"id": 10, "group": "Група 1", "name": "Кардаш Станіслав Анатолійович", "phone": "Не вказано", "car_type": "hyundai", "total_hours": 40, "spent_hours": 18, "paid_hours": 18, "school_payments": [{"hours": 18, "amount": 18 * 900, "date": "01.08.2026"}], "rate_type": "old", "medical_photo": None, "telegram_id": None}
    ]
...
async def link_student_start_callback(callback: CallbackQuery):
    ...
    kb_list = [[InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')})", callback_data=f"link_st_id_{s['id']}")] for s in unlinked]
    kb_list.append([InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_start_menu")])
...
async def back_to_start_menu_callback(callback: CallbackQuery):
    try:
    ...
    await callback.message.answer("Оберіть дію:", reply_markup=kb)
    await callback.answer()
...
async def student_my_progress(message: Message):
    ...
    group_val = student.get("group", "Група 1")
    phone_val = student.get("phone", "Не вказано")
...
async def export_excel_report(message: Message):
    ...
    writer.writerow(["ID", "Група", "ПІБ учня", "Телефон", "Авто", "Всього годин", "Від'їздено годин", "Оплачено годин", "Медична довідка"])
    ...
    for s in students:
        ...
        group_val = s.get("group", "Група 1")
        phone_val = s.get("phone", "Не вказано")
        writer.writerow([
            s["id"],
            group_val,
            s["name"],
    ...
    await message.answer_document(
        file, 
        caption="📊 **Ось звіт по учнях, групах та їхньому прогресу у форматі CSV** (можна вільно відкривати в Microsoft Excel або Google Таблицях).",
        parse_mode="Markdown"
    )

# ==========================================
# ДОДАВАННЯ ТА РОБОТА З УЧНЯМИ ТА МЕД. ДОВІДКАМИ
# ==========================================
...
async def process_add_student_btn(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введіть номер навчальної групи учня (наприклад, `Група 1` або `А-1`):\n*(або натисніть кнопку скасування нижче)*", 
        reply_markup=kb,
        parse_mode="Markdown"
    )
...
async def process_student_group(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_group")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await message.answer(
...
@router.callback_query(F.data == "back_to_add_student_group")
async def back_to_add_student_group(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AddStudent.group)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await callback.message.edit_text("Введіть номер навчальної групи учня (наприклад, `Група 1`):\n*(або натисніть кнопку скасування нижче)*", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def process_student_name(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_name")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await message.answer(
        "Введіть контактний номер телефону учня (наприклад, `+380671234567`):\n*(або натисніть кнопку скасування нижче)*",
        reply_markup=kb,
        parse_mode="Markdown"
    )
    await state.set_state(AddStudent.phone)

@router.callback_query(F.data == "back_to_add_student_name")
async def back_to_add_student_name(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AddStudent.name)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_group")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await callback.message.edit_text("Введіть ПІБ нового учня:\n*(або натисніть кнопку скасування нижче)*", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def process_student_phone(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_phone")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await message.answer(
...
@router.callback_query(F.data == "back_to_add_student_phone")
async def back_to_add_student_phone(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AddStudent.phone)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_name")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
    await callback.message.edit_text("Введіть контактний номер телефону учня (наприклад, `+380671234567`):\n*(або натисніть кнопку скасування нижче)*", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def process_student_car(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_add_student_car")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_add_student")]
    ])
...
@router.callback_query(F.data == "back_to_add_student_car")
async def back_to_add_student_car(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AddStudent.car_type)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Citroen (Механіка)", callback_data="car_citroen")],
        [InlineKeyboardButton(text="Hyundai Accent (Автомат)", callback_data="car_hyundai")],
    ...
    await callback.message.edit_text("Оберіть авто (тип коробки передач) для учня (40 годин курсу):\n*(Тариф: 900 грн/год)*", reply_markup=kb, parse_mode="Markdown")
    await callback.answer()
...
async def view_medical_certificates_msg(message: Message):
    ...
    for s in students:
        ...
        kb_list.append([InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')}) — [{status}]", callback_data=f"med_set_{s['id']}")])
...
async def med_set_callback(callback: CallbackQuery, state: FSMContext):
    ...
    text = f"👤 Учень: **{student['name']}** (Група: {student.get('group', 'Група 1')})\nМедична довідка: {status_text}\n\nОберіть потрібну дію:"
    ...
    kb_rows.append([InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="med_back_to_list")])
...
async def med_del_callback(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="med_back_to_list")]
    ])
...
async def process_delete_student_btn(message: Message):
    ...
    kb_list = [[InlineKeyboardButton(text=f"🗑 {s['name']} ({s.get('group', 'Група 1')})", callback_data=f"del_st_{s['id']}")] for s in students]
    kb = InlineKeyboardMarkup(inline_keyboard=kb_list)
...
async def view_students_list_msg(message: Message):
    ...
    # Групуємо учнів за групами
    groups_dict = {}
    for s in students:
        g = s.get("group", "Група 1")
        if g not in groups_dict:
            groups_dict[g] = []
        groups_dict[g].append(s)

    msg = "👥 **СПИСОК УЧНІВ ТА ЇХ ПРОГРЕС ЗА ГРУПАМИ**\n\n"
    ...
    for group_name, g_students in sorted(groups_dict.items()):
        msg += f"📚 **Навчальна група: {group_name}**\n"
        for i, s in enumerate(g_students, 1):
            car_lbl = "Citroen (МКПП)" if s["car_type"] == "citroen" else "Hyundai Accent (АКПП)"
            ...
            msg += (
                f"  {i}. **{s['name']}**\n"
                f"     📞 Тел: {phone_val} | 🚗 Авто: {car_lbl}\n"
                f"     ⏱ Від'їздив: {fmt(s['spent_hours'])} з {fmt(s['total_hours'])} год | "
                f"Оплачено учнем: {fmt(student_paid_hours)} год | "
...
async def start_student_payment(callback: CallbackQuery, state: FSMContext):
    ...
    kb_list = [[InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')})", callback_data=f"pay_st_{s['id']}")] for s in students]
    kb = InlineKeyboardMarkup(inline_keyboard=kb_list)
...
async def process_payment_student_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(student_id=st_id)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="add_student_payment")]
    ])
    await callback.message.answer("Введіть кількість годин, які сплатив учень (наприклад, `10` або `20`):", reply_markup=kb, parse_mode="Markdown")
...
async def view_students_payments_callback(callback: CallbackQuery):
    ...
    students = load_json("students", [])
    msg = "📜 **ІСТОРІЯ ОПЛАТ УЧНІВ АВТОШКОЛИ (КАСА):**\n\n"
    has_payments = False
    ...
    for s in students:
        ...
        if payments:
            has_payments = True
            msg += f"👤 **{s['name']}** ({s.get('group', 'Група 1')}):\n"
            for p in payments:
...
async def sch_add_start_msg(message: Message, state: FSMContext):
    ...
    kb_list = [[InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')})", callback_data=f"sch_st_{s['id']}")] for s in students]
    kb = InlineKeyboardMarkup(inline_keyboard=kb_list)
...
async def sch_choose_student(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(student_id=st_id)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="sch_back_to_students")]
    ])
    await callback.message.answer("Введіть дату заняття у форматі **ДД.ММ.РРРР** (наприклад, *15.08.2026*):", reply_markup=kb, parse_mode="Markdown")
    await state.set_state(ScheduleLesson.date)
    await callback.answer()

@router.callback_query(F.data == "sch_back_to_students")
async def sch_back_to_students(callback: CallbackQuery, state: FSMContext):
    students = load_json("students", [])
    kb_list = [[InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')})", callback_data=f"sch_st_{s['id']}")] for s in students]
    kb = InlineKeyboardMarkup(inline_keyboard=kb_list)
    await callback.message.answer("Оберіть учня для запису на заняття:", reply_markup=kb)
    await state.set_state(ScheduleLesson.student_id)
    await callback.answer()
...
async def start_daily_report_msg(message: Message, state: FSMContext):
    ...
    await state.update_data(today_sessions=[], citroen_last=citroen_last, hyundai_last=hyundai_last)
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Не змінився ({citroen_last} км)", callback_data="citroen_unchanged")],
        [InlineKeyboardButton(text="✏️ Ввести новий пробіг", callback_data="input_citroen_mil")],
...
@router.callback_query(F.data == "citroen_unchanged")
async def citroen_unchanged_callback(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    citroen_last = data.get("citroen_last", 0)
    await state.update_data(citroen_mil=citroen_last)
    
    hyundai_last = data.get("hyundai_last", 0)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Не змінився ({hyundai_last} км)", callback_data="hyundai_unchanged")],
        [InlineKeyboardButton(text="✏️ Ввести новий пробіг", callback_data="input_hyundai_mil")],
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_citroen_mil")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_daily_report")]
    ])
    await callback.message.edit_text(
...
@router.callback_query(F.data == "back_to_citroen_mil")
async def back_to_citroen_mil_callback(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    citroen_last = data.get("citroen_last", 0)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Не змінився ({citroen_last} км)", callback_data="citroen_unchanged")],
...
async def input_citroen_mil_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введіть поточний **загальний пробіг** авто **Citroen (МКПП)** в кілометрах (тільки число):", reply_markup=kb, parse_mode="Markdown")
...
@router.callback_query(F.data == "hyundai_unchanged")
async def hyundai_unchanged_callback(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    hyundai_last = data.get("hyundai_last", 0)
    await state.update_data(hyundai_mil=hyundai_last)
    
    students = load_json("students", [])
    if not students:
        await callback.message.edit_text("У системі немає учнів. Зверніться до власників.")
        await state.clear()
        await callback.answer()
        return
    ...
    kb_list = [[InlineKeyboardButton(text=f"{s['name']} ({s.get('group', 'Група 1')})", callback_data=f"rep_st_{s['id']}")] for s in students]
    kb_list.append([InlineKeyboardButton(text="🏁 Переглянути звіт (Готово)", callback_data="preview_report_flow")])
    kb_list.append([InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_hyundai_mil")])
    kb = InlineKeyboardMarkup(inline_keyboard=kb_list)
    ...
    full_text = f"{summary}\nОберіть учня, який займався сьогодні:"
    await callback.message.edit_text(full_text, reply_markup=kb, parse_mode="Markdown")
    await callback.answer()

@router.callback_query(F.data == "back_to_hyundai_mil")
async def back_to_hyundai_mil_callback(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    hyundai_last = data.get("hyundai_last", 0)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"✅ Не змінився ({hyundai_last} км)", callback_data="hyundai_unchanged")],
...
async def input_hyundai_mil_callback(callback: CallbackQuery, state: FSMContext):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_hyundai_mil")]
    ])
    await callback.message.edit_text("Введіть поточний **загальний пробіг** авто **Hyundai Accent (АКПП)** в кілометрах (тільки число):", reply_markup=kb, parse_mode="Markdown")
...
async def choose_student_for_report(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_student_choice")],
        [InlineKeyboardButton(text="❌ Скасувати", callback_data="cancel_daily_report")]
    ])
...
@router.callback_query(F.data == "back_to_student_choice")
async def back_to_student_choice_callback(callback: CallbackQuery, state: FSMContext):
    await state.set_state(DailyReport.student_choice)
    await send_student_menu(callback, state, "Оберіть учня, який займався сьогодні:")
    await callback.answer()
...
async def preview_daily_report(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⬅️ НАЗАД до вибору учнів", callback_data="back_to_student_choice")],
...
async def confirm_and_send_report(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.answer("✅ Щоденний звіт успішно збережено та надіслано!", reply_markup=kb)
    await state.clear()
    await callback.answer()

# --- ЗАПУСК БОТА ТА WEB-SERVER ---
async def handle_ping(request):
    return aiohttp.web.Response(text="Bot is running!")

async def main():
    if not TOKEN:
        print("Помилка: BOT_TOKEN не задано в змінних середовища!")
        return
    ...
    print(f"Вебсервер запущено на порті {port}. Бот стартує...")
