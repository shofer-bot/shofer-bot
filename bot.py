...
async def process_student_photo_text(message: Message, state: FSMContext):
    ...
    medical_photo = None
    if text in ["пропустити", "немає", "-"]:
        medical_photo = None
    else:
        medical_photo = None
    await save_new_student_to_db(message, state, medical_photo=medical_photo)
...
async def view_medical_certificates_msg(message: Message):
    ...
    await message.answer(
        ...
        "Оберіть учня нижче, щоб переглянути його поточну довідку, видалити її або завантажити/змінити в будь-який момент:",
        reply_markup=kb,
        parse_mode="Markdown"
    )
...
async def med_set_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if student.get("medical_photo"):
        text += "📸 У учня **вже є** збережена довідка. Ви можете переглянути її, **видалити** або надіслати **нове фото**, щоб замінити стару."
        kb_rows.append([InlineKeyboardButton(text="👀 Переглянути поточну довідку", callback_data=f"med_view_{st_id}")])
        kb_rows.append([InlineKeyboardButton(text="🗑 Видалити довідку", callback_data=f"med_del_{st_id}")])
    else:
        text += "❌ У учня **ще немає** довідки в системі.\n\nНадішліть **фото** нової медичної довідки сюди в чат (або напишіть `скасувати` для виходу)."

    kb_rows.append([InlineKeyboardButton(text="❌ Скасувати", callback_data="med_cancel")])
...
async def med_del_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🗑 Медичну довідку для учня **{student_name}** успішно видалено.", parse_mode="Markdown")
...
async def process_new_medical_photo_text(message: Message, state: FSMContext):
    ...
    if text in ["скасувати", "відміна", "назад", "exit"]:
        ...
        await message.answer("Будь ласка, надішліть **фотографію** медичної довідки або напишіть `скасувати`.")
...
@router.message(F.text == "👥 Учні та прогрес")
...
@router.message(F.text == "📅 Графік занять")
async def view_schedule_admin_msg(message: Message):
    if not is_authorized(message.from_user.id) or not is_admin(message.from_user.id): return
...
@router.message(F.text == "🚗 Авто та пробіг")
...
@router.message(F.text == "📋 Звіти інструктора")
...
@router.message(F.text == "➕ Записатися на заняття")
...
# --- ЩОДЕННИЙ ЗВІТ ---
@router.message(F.text == "🏁 Завершити день (Звіт)")
...
async def main():
    ...
    # Запускаємо вебсервер для Render у фоновому режимі[cite: 1]
